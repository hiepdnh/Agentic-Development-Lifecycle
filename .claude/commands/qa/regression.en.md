...
Create a regression test checklist before release, analyze blast radius, determine go/no-go. Triggers when: user says "regression test", "test before release", "verify no regression", "sprint sign-off", "QA before production deploy", or types /qa:regression.
