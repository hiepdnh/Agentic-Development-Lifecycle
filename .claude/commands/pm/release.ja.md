...
マージ済み PR + クローズ済み Issue から自動でリリースノートを生成。JP 顧客またはステークホルダーへの各リリース前に使用。 トリガー: 「リリースノートを作成」「リリースノートを書く」「スプリント changelog」「JP 顧客向けリリースノート」「リリース変更まとめ」、または /pm:release と入力時。
