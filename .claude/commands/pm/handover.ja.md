...
包括的なプロジェクト引き継ぎパッケージ（引き継ぎ書）を作成: コードベースマップ、決定ログ、用語集、オープン Issue、連絡先マトリクス。開発者離任、プロジェクト譲渡、JP 受託契約終了時に使用。 トリガー: 「プロジェクトを引き継ぐ」「引き継ぎ書を作成」「引き継ぎ」「開発者離任で引き継ぎ必要」「プロジェクト譲渡」「project handoff」、または /pm:handover と入力時。
