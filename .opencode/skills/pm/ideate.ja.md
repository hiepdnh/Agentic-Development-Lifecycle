...
name: pm:ideate
description: >
  曖昧なアイデアを明確なコンセプトに精製、問題ステートメントと NOT Doing リスト付き。/ba:spec の前に使用、初期からの仕様ミスを回避。
  トリガー: 「アイデアを探求」「方向性が不明」「機能をブレインストーミング」「アイデアが曖昧」「方針を明確化」、または /pm:ideate と入力時。
---

# /pm:ideate
**ロール**: PM / BA / Product Owner
**目的**: 曖昧なアイデアを明確なコンセプトに精製、`/ba:spec` 前に問題ステートメント + NOT Doing リストを確立。
