...
<!-- Gate: Xác định mục tiêu reverse -->
question({
  questions: [
    {
...
Dùng **explorer subagent** (read-only, fast) để khảo sát song song:
...
<!-- Gate cuối: Review baseline docs -->
question({
  questions: [{
    question: "Baseline docs đã tạo. Confirm?",
    header: "Finalize",
    options: [
      { label: "Xác nhận", description: "Baseline docs hoàn tất" },
...
- **Luôn dùng explorer subagent** cho scan — không scan trong main context để tránh nhồi token
